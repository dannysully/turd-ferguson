 ...
 *
 * The domain on the scan form is typed by whoever is asking, `readSite` fetches
 * it server side, and the prose that comes back is summarised by the model into
 * `brand_name` and `positioning` - both of which are returned to the person who
 * typed the address. That is a read of any URL our function can reach, with the
 * result handed back: an internal admin page, a staging host, or a cloud
