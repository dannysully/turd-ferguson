 ...
 * is reading the rule that ships. That distinction is not theoretical on this
 * file: the comment in `next.config.ts` records `/scan/:path*` once matching
 * bare `/scan` when `:path+` was meant, and only the compiled form tells the
 * two apart.
 */
export function noindexHeaderRules(manifest: string): { source: string; regex: RegExp }[] {
  ...
  return (parsed.headers ?? [])
    .filter((rule) =>
      rule.headers.some((h) => h.key.toLowerCase() === "x-robots-tag" && /noindex/.test(h.value)),
    )
...
export function disallowedPaths(robots: string): string[] {
  // Deliberately re-stated rather than imported from `sitemap.test.mts`:
  // importing a test module registers its tests, and this file would run the
  // sitemap suite a second time under its own name.
